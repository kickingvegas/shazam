...
#+TEXINFO_DIR_DESC: shazam.el is an Emacs interface to the Shazam music recognition feature for macOS.
...
#+VINDEX: shazam-log-file

* Usage
#+CINDEX: Usage

Identifying the current music playing is done with the command ~shazam~. Run it using your preferred key binding or via {{{kbd(M-x)}}}.
...
The command ~shazam-history~ will open the history log file (specified by the variable ~shazam-log-file~) of recognized music tracks.
